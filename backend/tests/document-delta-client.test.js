...
test('todo tick keeps complete operation after advancing a recurring task', () => {
  ...
  assert.match(todosSource, /clickedDoneOccurrence/);
  assert.match(todosSource, /!_todoRemainsOpenToday\(t\) \? todaySnap : null/);
  ...
  assert.match(appSource, /function _todoDeltaDrainBlocked\(/);
  assert.match(appSource, /function _stopDurableTodoDeltaAfterConflict\(/);
  ...
  assert.match(appSource, /opposite && String\(item\?\.operation \|\| ''\) === opposite/);
  assert.match(appSource, /_todoDeltaEpoch/);
  assert.match(appSource, /function _notifyTodoCreated\(/);
...
test('nested knowledge hashes restore the same folder on phone and laptop', () => {
  ...
  assert.match(appSource, /if \(finance && prevHashes\[id\] && !_pendingBusinessDeltaIds\(key\)\.has\(id\)\) return/);
...
test('manual backup restore compresses before chunking and retries server confirmation', () => {
  ...
  assert.match(appSource, /window\._avoidFullDocumentSync = false;\s*_forceNextServerSync\(\);/);
  assert.match(appSource, /const manualRestore = !!window\._manualRestoreSyncActive/);
});
