...
test('client assets are consistently bumped to tp144', () => {
  assert.match(app, /TP_ASSET_V\s*=\s*'tp144'/);
  assert.match(app, /team-pulse-static-v144/);
  assert.match(html, /app\.js\?v=tp144/);
  assert.match(sw, /team-pulse-static-v144/);
});
